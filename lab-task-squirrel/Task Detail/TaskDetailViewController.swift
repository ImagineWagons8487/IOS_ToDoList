...
class TaskDetailViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func didTapOpenCameraButton(_ sender: Any) {
        ...
        guard UIImagePickerController.isSourceTypeAvailable(.camera) else {
            // make it clear that it is not available
            let alert = UIAlertController(
                title: "Camera not available",
                message: "This deivce does not have a usable camera.",
        ...
        // create the camera picker object
        let picker = UIImagePickerController()
        
        // choose source type
        picker.sourceType = .camera
        
        // start mode is for photos
        picker.cameraCaptureMode = .photo
        
        // no edit screen after taking photo
        picker.allowsEditing = false
        
        // determine who receives the result, make sure to have this line
    ...
    @IBAction func didTapAttachPhotoButton(_ sender: Any) {
        ...
        // If the user is authorized, show the photo picker, otherwise request auth
        // If auth is denied, show an alert with an option to go to settings to update auth
        if PHPhotoLibrary.authorizationStatus(for: .readWrite) != .authorized {
            // Request photo library access
            PHPhotoLibrary.requestAuthorization(for: .readWrite) { [weak self] status in
                switch status {
                case .authorized:
                    // User authorized access to their photo library
    ...
    private func presentImagePicker() {
        ...
        // Request the original file format, this is the fastest method because it avoids transcoding
        config.preferredAssetRepresentationMode = .current
        
        // Only allow a single image to be selected at a time
            // can we set it to anything we want with just this line? Code probably is built for receiving one image at a time
        config.selectionLimit = 1
        
        // Instantiate a picker while passing in the config
        let picker = PHPickerViewController(configuration: config)
        
        // Set the picker delegate so we can receive whatever image the user picks
        picker.delegate = self
        
        // Present the picker, pretty much show it
        present(picker, animated: true)
    }
    ...
    func updateMapView() {
        ...
        // Get the coordinate form the iamge location, this is the lat/long of the location
        let coordinate = imageLocation.coordinate
        
        // Set the map view's region based on the coordinate of the image
        // The span represents the map's "zoom level". A smaller value yields a bigger zoom
        let region = MKCoordinateRegion(center: coordinate, span: MKCoordinateSpan(latitudeDelta: 0.01, longitudeDelta: 0.01))
        
        mapView.setRegion(region, animated: true)
        
        // Add an annotation to the map view base don image location
...
extension TaskDetailViewController: PHPickerViewControllerDelegate {
    ...
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        // Get the selected image asset (we can grab the 1st item in the array since we can only do one at a time)
        let result = results.first
        
        // Get image location
        // PHAsset contains medtadata about an image or video (ex. location, size, etc.)
...
extension TaskDetailViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        guard let annotationView = mapView.dequeueReusableAnnotationView(withIdentifier: TaskAnnotationView.identifier) as? TaskAnnotationView else {
...
extension TaskDetailViewController {
    ...
    // runs when the user taps the cancel button in the camera UI
    func imagePickerControllerDidCancel(_ picker: UIImagePickerController) {
        // closes camera screen and goes back to the previous screen
        picker.dismiss(animated: true)
    }
    
    // runs after user takes a photo and taps "Use Photo"
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        // this pulls the original, undedited image out of the info dictionary
